#Setting the work directory by going on Session
#Loading and Viewing the three data files
...
#Creating a variable DATE of class "Date" using as.Date function
DATE<-as.Date(as.character(Market$V1), "%Y%m%d")


#Checking the dimensions of "Case3Port" (All 3 data files)
dim(Market)
dim(Port)
dim(FirmSECID)


#Assigning columns and rows of the Port data file
ncol_Port<-ncol(Port)
nrow_Port<-nrow(Port)
...
#Step1 - Drop Firms
#Calculating the number of Nan for each firm to drop it later
...
#Finding the firms which have zero observations in the whole sample period
vec_DELETE<-which(num_Nan == nrow_Port)


#Deleting the firms with zero observations in the whole sample period 
...
#Descriptive statistics for IBM
...
#Constructing a table to present all the results
...
FirmReturn_50Q <- Firm_RET[,TICK_vec[3]]
FirmReturn_50Q

#Plotting Histogram
jpeg(filename = "Case3Histogram.jpeg")
hist(FirmReturn_50Q, main = "Daily Market Excess returns (percentage)", prob=TRUE, breaks = 50)
curve(dnorm (x, mean = mean(FirmReturn_50Q), sd = sd(FirmReturn_50Q)), add=TRUE)
dev.off()

#Q-Q Plot
jpeg(filename = "Case3QQPlot.jpeg")
qqnorm(FirmReturn_50Q, main="Q-Q plot of Rock Excess Returns")
qqline(FirmReturn_50Q)
dev.off()

# Jarque Bera test
library(tseries)
jarque.bera.test(FirmReturn_50Q)

#Lillefors Test
library(nortest)
lillie.test(FirmReturn_50Q)
